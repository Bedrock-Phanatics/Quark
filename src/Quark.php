...
namespace quark {
	...
	use quark\utils\Timezone;
	use quark\utils\Utils;
	use quark\wizard\SetupWizard;
	...
	function check_platform_dependencies(){
		...
		$extensions = [
			"chunkutils2" => "Quark ChunkUtils v2",
			"curl" => "cURL",
			"crypto" => "php-crypto",
			"ctype" => "ctype",
			"date" => "Date",
		...
		if(($leveldb_version = phpversion("leveldb")) !== false){
			...
			if(!defined('LEVELDB_ZLIB_RAW_COMPRESSION')){
				$messages[] = "Given version of php-leveldb doesn't support ZLIB_RAW compression (use https://github.com/Bedrock-Phanatics/php-leveldb)";
			}
		}
		...
		if(extension_loaded("quark")){
			$messages[] = "The native Quark extension is no longer supported.";
		}
...
JIT_WARNING
	...
	function server(){
		if(count($messages = check_platform_dependencies()) > 0){
			...
			critical_error("Please recompile PHP with the needed configuration, or refer to the installation instructions at http://quark.rtfd.io/en/rtfd/installation.html.");
			echo PHP_EOL;
			exit(1);
		}
		unset($messages);
		...
		$composerGitHash = InstalledVersions::getReference('quark/quark');
		...
		if(defined('quark\ORIGINAL_PHAR_PATH')){
			//if we're inside a phar cache, \quark\PATH will not include the original phar
